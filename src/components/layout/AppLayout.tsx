...
export function AppLayout({children}:AppLayoutProps){
  ...
  return(
    <div className="flex min-h-screen bg-background">
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 w-64 border-r border-border/50 bg-sidebar transition-transform duration-300',
      ...
      <main className="flex-1 md:ml-64">
        ...
        {/* Top Bar */}
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/50 bg-background/80 px-4 md:px-8 backdrop-blur-lg">
          <div className="flex items-center gap-4">
            ...
            <Activity className="h-5 w-5 text-success hidden sm:block"/>
...
function NotificationBell(){
  ...
  React.useEffect(()=>{
    ...
    const fetchUnread=async()=>{
      const {count}=await import('@/lib/api').then(m=>m.notificationsApi.unreadCount())
