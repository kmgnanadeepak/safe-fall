...
export default function EmergencyContacts() {
  ...
  const fetchContacts = async () => {
    if (!user?.id) return;
    const { data, error } = await emergencyContactsApi.list();
    if (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const { error } = await emergencyContactsApi.update(editingId, formData);
      if (error) {
      ...
      const { error } = await emergencyContactsApi.create(formData);
      if (error) {
  ...
  const handleDelete = async (id: string) => {
    if (!user?.id) return;
    const res = await emergencyContactsApi.delete(id);
