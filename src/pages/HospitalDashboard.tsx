...
import { falleventsApi, notificationsApi } from '@/lib/api';
...
export default function HospitalDashboard() {
  ...
  const fetchEmergencies = async () => {
    if (!user?.id) return;
    const { data: list, error } = await falleventsApi.listHospitalDashboard();
    if (error) {
      console.error('Error fetching emergencies:', error);
      setLoading(false);
      return;
    }
    const enrichedEmergencies: ActiveEmergency[] = (list || []).map((e: Record<string, unknown>) => ({
      id: String(e.id),
      user_id: String(e.user_id),
      timestamp: String(e.timestamp),
      latitude: (e.latitude as number) ?? null,
      longitude: (e.longitude as number) ?? null,
      patient_name: String((e as { patient_name?: string }).patient_name || 'Unknown'),
      patient_email: String((e as { patient_email?: string }).patient_email || ''),
      health_profile: (e as { health_profile?: ActiveEmergency['health_profile'] }).health_profile,
    }));
  ...
  const handleResolve = async (emergencyId: string, patientUserId: string) => {
    ...
    const updateRes = await falleventsApi.update(emergencyId, {
      resolved: true,
            ...
            <div className="space-y-4">
              {emergencies.map((emergency, index) => (
                ...
                >
                  ...
                  {/* Quick Map Preview - pass this patient's coordinates so map opens centered on them */}
                  {emergency.latitude != null && emergency.longitude != null && (
                    <div className="border-t border-border/50 p-4">
                      <Link
                        to="/hospital-map"
                        state={{
                          focusLat: emergency.latitude,
                          focusLng: emergency.longitude,
                          focusPatientId: emergency.id,
                          focusPatientName: emergency.patient_name,
                        }}
